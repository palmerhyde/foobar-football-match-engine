...
function validateCombo(json) {
    var value = false;

    if (json) {
        var result = comboSchema.schema.validate(json);
        value = result.error === undefined;
    }
...
function validateSquad(json) {
    ...
    if (!result.value) {
        return { 'error': { 'message' : 'Squad is undefined' }};
    }

    if (!result.error) {
        const message = validNumberOfPlayersPerPosition(json);
