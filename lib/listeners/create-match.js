var firebase = require('../services/firebase')
var { CreateMatch } = require('../game/create-match')
var config = require('../config');

async function listenToCreateMatchReference() {
    console.log('Listener: Create Match');
    const db = firebase.Database()
    const ref = db.ref(config.CreateMatchReference);
    ref.on('child_added', async (snapshot) => {
        const data = snapshot.val();
      
        var matchRef = db.ref(config.MatchReference);
        var userRef = db.ref(config.MatchViewReference + '/' + data.homeUserId)
        const userSnapshot = await userRef.once('value').value;

        if (!userSnapshot) {
            const match = await CreateMatch(data.homeUserId, data.awayUserId)
        
            matchRef.push(match, () => {
                console.log('Match added:' + match.homeTeam.clubName + ' VS ' + match.awayTeam.clubName)
                ref.child(snapshot.key).remove();
            })
        }
        else {
            console.log("\"" + homeUserId + "\" is already playing a match." )
        }
    });
  }

listenToCreateMatchReference();